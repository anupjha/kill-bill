...
const EditableFileImage = ({ className, placeholder, value, width, onChangeImage, onChangeWidth, pdfMode }) => {
  ...
  if (pdfMode) {
    if (value) {
      return <Image style={{ ...compose(`image ${className ? className : ""}`), maxWidth: width }} src={value} />;
    } else {
      return <></>;
    }
  }
  ...
  return (
    <div className={`image ${value ? "mb-5" : ""} ${className ? className : ""}`}>
      {!value ? (
        ...
        <>
          <img src={value} className="image__img" alt={placeholder} style={{ maxWidth: width || 100 }} />
